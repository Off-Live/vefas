...
fn maps_core_errors_to_shared_error_taxonomy() {
    ...
    match ve {
        VefasError::HttpError { error_type, .. } => assert_eq!(error_type, HttpErrorType::InvalidUrl),
    ...
    let ve: VefasError = e.into();
    match ve {
        VefasError::TlsError { error_type, .. } => assert_eq!(error_type, TlsErrorType::InvalidHandshake),
        _ => panic!("unexpected mapping"),
    }
}
