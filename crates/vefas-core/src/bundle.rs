...
use crate::{
    http::HttpData,
    keylog::VefasKeyLog,
    records::{HandshakeMessage, HandshakeType, TlsRecordParser},
    session::SessionData,
...
impl BundleBuilder {
    ...
    pub fn from_session_data(
        ...
        let (encrypted_request, encrypted_response) =
            self.extract_ciphertext_records_from_session(session)?;
        ...
        if handshake_components.certificate_msg.is_empty()
            || handshake_components.finished_msgs.is_empty()
        {
            if let Some((cert_opt, cert_verify_opt, finished_opt)) = self
                .try_decrypt_and_validate_server_finished(
                    session,
                    &secret_data,
                    &handshake_components.client_hello,
                    &handshake_components.server_hello,
                )?
            {
        ...
        let certificate_msg = if let Some(m) = recovered_certificate_msg {
            m
        } else {
            handshake_components.certificate_msg
        };
        let certificate_verify_msg = if let Some(m) = recovered_certificate_verify_msg {
            m
        } else {
            handshake_components.certificate_verify_msg
        };
        let finished_msgs = if let Some(m) = recovered_finished_msg {
            vec![m]
        } else {
            handshake_components.finished_msgs
        };
        ...
        for message in out_msgs {
            match message.msg_type {
                ...
                HandshakeType::Finished => {
                    ...
                    components
                        .other_messages
                        .push(self.serialize_handshake_message(&message)?);
                }
                _ => {
                    components
                        .other_messages
            ...
            match message.msg_type {
                ...
                HandshakeType::CertificateVerify => {
                    components.certificate_verify_msg =
                        self.serialize_handshake_message(&message)?;
                }
                HandshakeType::Finished => {
                    // server Finished
                    components
    ...
    fn extract_randoms_from_session(&self, session: &SessionData) -> Result<([u8; 32], [u8; 32])> {
        ...
        let client_random = client_random.ok_or_else(|| {
            VefasCoreError::ValidationError(
                "ClientHello not found in handshake messages".to_string(),
            )
        })?;

        let server_random = server_random.ok_or_else(|| {
            VefasCoreError::ValidationError(
                "ServerHello not found in handshake messages".to_string(),
            )
        })?;
    ...
    fn extract_random_from_hello(&self, payload: &[u8], is_client_hello: bool) -> Result<[u8; 32]> {
        ...
        if payload.len() < 34 {
            return Err(VefasCoreError::ValidationError(format!(
                "Hello message too short: {} bytes (min 34)",
                payload.len()
            )));
        }
        ...
        if offset >= payload.len() {
            return Err(VefasCoreError::ValidationError(
                "Hello missing legacy_session_id length".into(),
            ));
        }
        let sid_len = payload[offset] as usize;
        offset = offset
            .checked_add(1 + sid_len)
            .ok_or_else(|| VefasCoreError::ValidationError("Overflow parsing session_id".into()))?;
        if offset > payload.len() {
            return Err(VefasCoreError::ValidationError(
                "Hello legacy_session_id exceeds message size".into(),
            ));
        }
        ...
        if is_client_hello {
            ...
            if offset + 2 > payload.len() {
                return Err(VefasCoreError::ValidationError(
                    "ClientHello missing cipher_suites length".into(),
                ));
            }
            let cs_len = u16::from_be_bytes([payload[offset], payload[offset + 1]]) as usize;
            offset += 2;
            offset = offset.checked_add(cs_len).ok_or_else(|| {
                VefasCoreError::ValidationError("Overflow parsing cipher_suites".into())
            })?;
            if offset > payload.len() {
                return Err(VefasCoreError::ValidationError(
                    "ClientHello cipher_suites exceeds message size".into(),
                ));
            }
            ...
            if offset >= payload.len() {
                return Err(VefasCoreError::ValidationError(
                    "ClientHello missing compression_methods length".into(),
                ));
            }
            let cm_len = payload[offset] as usize;
            offset = offset.checked_add(1 + cm_len).ok_or_else(|| {
                VefasCoreError::ValidationError("Overflow parsing compression_methods".into())
            })?;
            if offset > payload.len() {
                return Err(VefasCoreError::ValidationError(
                    "ClientHello compression_methods exceeds message size".into(),
                ));
            }
            ...
            if offset + 2 > payload.len() {
                return Err(VefasCoreError::ValidationError(
                    "ClientHello missing extensions length".into(),
                ));
            }
            let ext_len = u16::from_be_bytes([payload[offset], payload[offset + 1]]) as usize;
            offset += 2;
            offset = offset.checked_add(ext_len).ok_or_else(|| {
                VefasCoreError::ValidationError("Overflow parsing ClientHello extensions".into())
            })?;
            if offset > payload.len() {
                return Err(VefasCoreError::ValidationError(
                    "ClientHello extensions exceed message size".into(),
                ));
            }
        } else {
            ...
            if offset + 2 > payload.len() {
                return Err(VefasCoreError::ValidationError(
                    "ServerHello missing cipher_suite".into(),
                ));
            }
            ...
            if offset + 1 > payload.len() {
                return Err(VefasCoreError::ValidationError(
                    "ServerHello missing compression_method".into(),
                ));
            }
            ...
            if offset + 2 > payload.len() {
                return Err(VefasCoreError::ValidationError(
                    "ServerHello missing extensions length".into(),
                ));
            }
            let ext_len = u16::from_be_bytes([payload[offset], payload[offset + 1]]) as usize;
            offset += 2;
            offset = offset.checked_add(ext_len).ok_or_else(|| {
                VefasCoreError::ValidationError("Overflow parsing ServerHello extensions".into())
            })?;
            if offset > payload.len() {
                return Err(VefasCoreError::ValidationError(
                    "ServerHello extensions exceed message size".into(),
                ));
            }
        }
    ...
    fn extract_ciphertext_records_from_session(
        &self,
        session: &SessionData,
        ...
        let req = TlsRecordParser::new()
            ...
            .ok_or_else(|| {
                VefasCoreError::ValidationError(
                    "No outbound ApplicationData record found".to_string(),
                )
            })?;
        let resp = TlsRecordParser::new()
            ...
            .ok_or_else(|| {
                VefasCoreError::ValidationError(
                    "No inbound ApplicationData record found".to_string(),
                )
            })?;
    ...
    fn create_bundle_with_strategy(
        &self,
        ...
        match self.compression_strategy {
            ...
            CompressionStrategy::Auto => {
                self.create_deterministic_bundle_with_compression(components)
            }
            CompressionStrategy::Always => {
                self.create_deterministic_bundle_force_compression(components)
            }
        }
    }

    /// Create the final deterministic bundle with automatic compression
    fn create_deterministic_bundle_with_compression(
        &self,
        components: BundleComponents,
    ) -> Result<VefasCanonicalBundle> {
        ...
        let client_private_key = components.client_ephemeral_private_key.ok_or_else(|| {
            VefasCoreError::ValidationError(
                "Missing client ephemeral private key; configure provider seed or capture hooks"
                    .to_string(),
            )
        })?;
        ...
        let bundle = VefasCanonicalBundle::new_with_compression(
            ...
            components
                ...
                .into_iter()
                .next()
                .unwrap_or_default(), // server_finished_msg
            client_private_key,
        ...
        .map_err(|e| {
            VefasCoreError::ValidationError(format!("Failed to create bundle: {:?}", e))
        })?;
    ...
    fn create_deterministic_bundle_force_compression(
        &self,
        components: BundleComponents,
    ) -> Result<VefasCanonicalBundle> {
        ...
        // Force compression
        bundle.try_compress().map_err(|e| {
            VefasCoreError::ValidationError(format!("Failed to force compression: {:?}", e))
        })?;
    ...
    fn create_deterministic_bundle(
        &self,
        components: BundleComponents,
    ) -> Result<VefasCanonicalBundle> {
        // Generate deterministic verifier nonce based on bundle contents
    ...
    fn try_decrypt_and_validate_server_finished(
        ...
        let server_hs_secret = match secrets
            .traffic_secrets
            .iter()
            .find(|s| s.label == "SERVER_HANDSHAKE_TRAFFIC_SECRET")
        {
        ...
        if secret_len == 32 {
            let hk = Hkdf::<Sha256>::from_prk(&server_hs_secret)
                .map_err(|_| VefasCoreError::ValidationError("invalid PRK".into()))?;
            let mut key = [0u8; 16];
            ...
            label_key.push(0);
            hk.expand(&label_key, &mut key)
                .map_err(|_| VefasCoreError::ValidationError("HKDF expand key failed".into()))?;
            ...
            label_iv.push(0);
            hk.expand(&label_iv, &mut iv_static)
                .map_err(|_| VefasCoreError::ValidationError("HKDF expand iv failed".into()))?;

            // Decrypt with AES-128-GCM
            let cipher = Aes128Gcm::new_from_slice(&key)
                .map_err(|_| VefasCoreError::ValidationError("AES key error".into()))?;
            let (recovered_ee, recovered_cert, recovered_cv, recovered_fin) =
                process_records(&cipher, records.clone(), &mut iv_static);
            // Validate finished if found using SHA-256
            if let Some(fin) = &recovered_fin {
                self.verify_server_finished_with_hash::<Sha256>(
                    &server_hs_secret,
                    client_hello,
                    server_hello,
                    recovered_ee.as_ref(),
                    recovered_cert.as_ref(),
                    recovered_cv.as_ref(),
                    fin,
                )?;
            }
            return Ok(Some((
                recovered_cert,
                recovered_cv,
                recovered_fin.unwrap_or_default(),
            )));
        } else {
            let hk = Hkdf::<Sha384>::from_prk(&server_hs_secret)
                .map_err(|_| VefasCoreError::ValidationError("invalid PRK".into()))?;
            let mut key = [0u8; 32];
            ...
            if let Some(fin) = &recovered_fin {
                self.verify_server_finished_with_hash::<Sha384>(
                    &server_hs_secret,
                    client_hello,
                    server_hello,
                    recovered_ee.as_ref(),
        ...
        fn process_records<
            C: AeadInPlace<
                NonceSize = aes_gcm::aead::consts::U12,
                TagSize = aes_gcm::aead::consts::U16,
            >,
        >(
            cipher: &C,
            ...
            let mut seq: u64 = 0;
            let mut recovered_ee: Option<Vec<u8>> = None;
            ...
            for rec in records {
                ...
                if cipher
                    .decrypt_in_place_detached(nonce_arr, &aad, &mut plaintext, tag)
                    .is_err()
                {
                    continue;
                }
                ...
                while idx + 4 <= plaintext.len() {
                    ...
                    let len = ((plaintext[idx + 1] as usize) << 16)
                        | ((plaintext[idx + 2] as usize) << 8)
                        | (plaintext[idx + 3] as usize);
                    idx += 4;
                    if idx + len > plaintext.len() {
                        break;
                    }
                    ...
                    match typ {
                        8 => {
                            if recovered_ee.is_none() {
                                // EncryptedExtensions - CRITICAL for Server Finished transcript
                                let mut msg = vec![
                                    8,
                                    ((len >> 16) & 0xff) as u8,
                                    ((len >> 8) & 0xff) as u8,
                                ...
                                recovered_ee = Some(msg);
                            }
                        }
                        11 => {
                            if recovered_cert.is_none() {
                                // Certificate
                                let mut msg = vec![
                                    11,
                                    ((len >> 16) & 0xff) as u8,
            ...
            (recovered_ee, recovered_cert, recovered_cv, recovered_fin)
        }
    ...
    fn verify_server_finished_with_hash<H: Digest + Default + Clone + 'static>(
        ...
        client_hello: &Vec<u8>,
        server_hello: &Vec<u8>,
        encrypted_extensions: Option<&Vec<u8>>,
        cert: Option<&Vec<u8>>,
        cert_verify: Option<&Vec<u8>>,
        finished: &Vec<u8>,
    ) -> Result<()> {
        // Build transcript over handshake messages up to (excluding) Finished
        // TLS 1.3 order: ClientHello, ServerHello, EncryptedExtensions, Certificate, CertificateVerify
        let mut transcript = Vec::new();
        transcript.extend_from_slice(client_hello);
        transcript.extend_from_slice(server_hello);
        // EncryptedExtensions MUST be included - this is critical for Server Finished validation!
        ...
        if out_len == 32 {
            ...
            label.push(0);
            hk.expand(&label, &mut finished_key).map_err(|_| {
                VefasCoreError::ValidationError("HKDF expand finished failed".into())
            })?;
            let mut mac = <Hmac<Sha256> as Mac>::new_from_slice(&finished_key)
                .map_err(|_| VefasCoreError::ValidationError("HMAC key error".into()))?;
            mac.update(&th);
            ...
            Err(VefasCoreError::ValidationError(
                "Unsupported hash length for Finished".into(),
            ))
        }
    }
    ...
    fn compare_finished(&self, finished: &Vec<u8>, expected: &[u8]) -> Result<()> {
        if finished.len() < 4 {
            return Err(VefasCoreError::ValidationError("Finished too short".into()));
        }
        let fin_len =
    ...
    fn extract_certificate_chain_from_certificate_msg(
        &self,
        cert_msg: &Vec<u8>,
    ) -> Result<Vec<Vec<u8>>> {
        ...
        let cert_list_length = u32::from_be_bytes([
            0,
            cert_data[offset],
            cert_data[offset + 1],
            cert_data[offset + 2],
        ...
        if offset + cert_list_length > cert_data.len() {
            return Err(VefasCoreError::ValidationError(
                "Certificate list length exceeds message size".to_string(),
            ));
        }
        ...
        while offset < cert_list_end {
            ...
            if offset + cert_length > cert_list_end {
                return Err(VefasCoreError::ValidationError(
                    "Certificate length exceeds remaining data".to_string(),
                ));
            }
            ...
            if offset + 2 <= cert_list_end {
                let ext_length =
                    u16::from_be_bytes([cert_data[offset], cert_data[offset + 1]]) as usize;
                offset += 2 + ext_length;
            }
        }
...
mod finished_tests {
    ...
    fn decrypt_and_validate_finished_sha256_succeeds() {
        ...
        let secrets = SecretData {
            ...
            traffic_secrets: vec![TrafficSecret {
                label: "SERVER_HANDSHAKE_TRAFFIC_SECRET".into(),
                secret: server_hs_secret,
                client_random: [0u8; 32],
            }],
        };
        ...
        let got = builder
            .try_decrypt_and_validate_server_finished(&session, &secrets, &ch, &sh)
            .unwrap();
        assert!(got.is_some());
    ...
    fn rfc8448_finished_vectors_dir() {
        ...
        let dir = std::env::var("RFC8448_FINISHED_DIR")
            .unwrap_or_else(|_| String::from("crates/vefas-core/tests/fixtures/rfc8448_finished"));
        let p = Path::new(&dir);
        if !p.exists() {
            return;
        }
        ...
        for entry in fs::read_dir(p).unwrap().flatten() {
            ...
            if v.hash.eq_ignore_ascii_case("SHA256") || server_hs_secret.len() == 32 {
                if let Some(fin) = fin_opt {
                    builder
                        .verify_server_finished_with_hash::<Sha256>(
                            &server_hs_secret,
                            &ch,
                            &sh,
                            None, // EncryptedExtensions - RFC 8448 vectors may not include this
                            cert.as_ref(),
                            cv.as_ref(),
                            &fin,
                        )
                        .expect("sha256 finished verify");
                } else {
                    // Compute expected Finished and validate
                    let mut transcript = Vec::new();
                    transcript.extend_from_slice(&ch);
                    transcript.extend_from_slice(&sh);
                    // Note: RFC 8448 test vectors may not include EncryptedExtensions
                    if let Some(c) = &cert {
                        transcript.extend_from_slice(c);
                    }
                    if let Some(cv) = &cv {
                    ...
                    let hk = Hkdf::<Sha256>::from_prk(&server_hs_secret).unwrap();
                    let mut label = Vec::new();
                    label.extend_from_slice(&u16::to_be_bytes(32));
                    label.push(6 + b"finished".len() as u8);
                    label.extend_from_slice(b"tls13 finished");
                    label.push(0);
                    let mut finished_key = [0u8; 32];
                    hk.expand(&label, &mut finished_key).unwrap();
                    let mut mac = <Hmac<Sha256> as Mac>::new_from_slice(&finished_key).unwrap();
                    mac.update(&th);
                    let verify_data = mac.finalize().into_bytes();
                ...
                if let Some(fin) = fin_opt {
                    builder
                        .verify_server_finished_with_hash::<Sha384>(
                            &server_hs_secret,
                            &ch,
                            &sh,
                            None, // EncryptedExtensions
                    ...
                    let hk = Hkdf::<Sha384>::from_prk(&server_hs_secret).unwrap();
                    let mut label = Vec::new();
                    label.extend_from_slice(&u16::to_be_bytes(48));
                    label.push(6 + b"finished".len() as u8);
                    label.extend_from_slice(b"tls13 finished");
                    label.push(0);
                    let mut finished_key = [0u8; 48];
...
mod tests {
    ...
    use rustls::{crypto::aws_lc_rs::cipher_suite, KeyLog};
    ...
    fn create_test_session_data() -> SessionData {
        ...
        let ch_record: Vec<u8> = vec![
            0x16, 0x03, 0x01, 0x00, 0xC4, 0x01, 0x00, 0x00, 0xC0, 0x03, 0x03, 0xCB, 0x34, 0xEC,
        ...
        let cert_payload: Vec<u8> = vec![
            ,
        ];
        ...
        let cert_verify_payload: Vec<u8> = vec![
            0x08, 0x04, 0x00, 0x80, 0x5A, 0x74, 0x7C, 0x5D, 0x88, 0xFA, 0x9B, 0xD2, 0xE5, 0x5A,
            0xB0, 0x85, 0xA6, 0x10, 0x15, 0xB7, 0x21, 0x1F, 0x82, 0x4C, 0xD4, 0x84, 0x14, 0x5A,
            0xB3, 0xFF, 0x52, 0xF1, 0xFD, 0xA8, 0x47, 0x7B, 0x0B, 0x7A, 0xBC, 0x90, 0xDB, 0x78,
            0xE2, 0xD3, 0x3A, 0x5C, 0x14, 0x1A, 0x07, 0x86, 0x53, 0xFA, 0x6B, 0xEF, 0x78, 0x0C,
            0x5E, 0xA2, 0x48, 0xEE, 0xAA, 0xA7, 0x85, 0xC4, 0xF3, 0x94, 0xCA, 0xB6, 0xD3, 0x0B,
            0xBE, 0x8D, 0x48, 0x59, 0xEE, 0x51, 0x1F, 0x60, 0x29, 0x57, 0xB1, 0x54, 0x11, 0xAC,
            0x02, 0x76, 0x71, 0x45, 0x9E, 0x46, 0x44, 0x5C, 0x9E, 0xA5, 0x8C, 0x18, 0x1E, 0x81,
            0x8E, 0x95, 0xB8, 0xC3, 0xFB, 0x0B, 0xF3, 0x27, 0x84, 0x09, 0xD3, 0xBE, 0x15, 0x2A,
            0x3D, 0xA5, 0x04, 0x3E, 0x06, 0x3D, 0xDA, 0x65, 0xCD, 0xF5, 0xAE, 0xA2, 0x0D, 0x53,
        ...
        let fin_payload: Vec<u8> = vec![
            0x9B, 0x9B, 0x14, 0x1D, 0x90, 0x63, 0x37, 0xFB, 0xD2, 0xCB, 0xDC, 0xE7, 0x1D, 0xF4,
    ...
    fn create_test_keylog() -> VefasKeyLog {
        ...
        keylog.log("CLIENT_HANDSHAKE_TRAFFIC_SECRET", &[1; 32], &[0xaa; 32]);
        keylog.log("SERVER_HANDSHAKE_TRAFFIC_SECRET", &[1; 32], &[0xbb; 32]);
    ...
    fn test_deterministic_bundle_creation() {
        ...
        assert_eq!(
            bundle1.client_hello().unwrap(),
            bundle2.client_hello().unwrap()
        );
        assert_eq!(
            bundle1.server_hello().unwrap(),
            bundle2.server_hello().unwrap()
        );
    }
