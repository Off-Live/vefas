...
fn test_ecdsa_certificate_validation() {
    ...
    // Test ECDSA P-384 certificate (higher security)
    if fixtures.certificate_exists("ecdsa_p384") {
        ...
        assert!(
            result.is_ok(),
            "ECDSA P-384 self-signed certificate verification should succeed. Error: {:?}",
            result.err()
        );
        
        println!("✅ ECDSA P-384 certificate validation passed");
    } else {
        println!("⚠️  ECDSA P-384 certificate not found, skipping test");
    }

    // Test ECDSA P-521 certificate (highest security)
    if fixtures.certificate_exists("ecdsa_p521") {
        ...
        assert!(
            result.is_ok(),
            "ECDSA P-521 self-signed certificate verification should succeed. Error: {:?}",
            result.err()
        );
        
        println!("✅ ECDSA P-521 certificate validation passed");
    } else {
