...
use sp1_sdk::{ProverClient, SP1ProofWithPublicValues, SP1Stdin, Prover};
...
// Import prover types for both CPU and CUDA
use sp1_sdk::env::EnvProver;
use sp1_sdk::cuda::CudaProver;
...
/// - Uses ProverClient::from_env() which returns EnvProver (CPU)
/// - Uses ProverClient::builder().cuda().build() which returns CudaProver (GPU)
...
pub struct VefasSp1Prover {
    /// SP1 prover (either CPU EnvProver or CUDA CudaProver)
    prover: Sp1ProverVariant,
}

/// Enum to handle both CPU and CUDA prover types
enum Sp1ProverVariant {
    Cpu(EnvProver),
    #[cfg(feature = "cuda")]
    Cuda(CudaProver),
}
...
impl VefasSp1Prover {
    ...
    /// Supports both CPU and CUDA acceleration based on features.
    pub fn new() -> Self {
        #[cfg(feature = "cuda")]
        {
            println!("SP1: Initializing CUDA-accelerated prover...");
            println!("SP1: Using GPU for proof generation (dramatically faster than CPU)");
            Self {
                prover: Sp1ProverVariant::Cuda(ProverClient::builder().cuda().build()),
            }
        }
        
        #[cfg(not(feature = "cuda"))]
        {
            println!("SP1: Initializing CPU prover...");
            println!("SP1: Tip: Enable 'cuda' feature for dramatically faster proof generation");
            Self {
                prover: Sp1ProverVariant::Cpu(ProverClient::from_env()),
            }
        }
    }
    ...
    pub fn generate_zk_proof(&self, bundle: &VefasCanonicalBundle) -> VefasResult<VefasSp1Proof> {
        ...
        // Setup proving/verification keys and execute based on prover type
        let (pk, _vk, execution_report) = match &self.prover {
            Sp1ProverVariant::Cpu(prover) => {
                let (pk, vk) = prover.setup(VEFAS_SP1_ELF);
                ...
                (pk, vk, execution_report)
            }
            #[cfg(feature = "cuda")]
            Sp1ProverVariant::Cuda(prover) => {
                let (pk, vk) = prover.setup(VEFAS_SP1_ELF);
                ...
                let (_, execution_report) = prover
                    ...
                    .run()
                    .map_err(|e| {
                        VefasError::zkvm_error("sp1", &format!("SP1 CUDA execution failed: {:?}", e))
                    })?;
                let _execute_time = execute_start.elapsed();
        ...
        // Generate proof based on prover type
        let proof_start = Instant::now();
        let mut proof = match &self.prover {
            Sp1ProverVariant::Cpu(prover) => {
                prover.prove(&pk, &stdin).run().map_err(|e| {
            ...
            Sp1ProverVariant::Cuda(prover) => {
                prover.prove(&pk, &stdin).run().map_err(|e| {
                    VefasError::zkvm_error("sp1", &format!("SP1 CUDA proof execution failed: {:?}", e))
                })?
            }
        };
    ...
    pub fn verify_proof(&self, proof: &VefasSp1Proof) -> VefasResult<VefasProofClaim> {
        ...
        // Setup verification key and verify proof based on prover type
        match &self.prover {
            Sp1ProverVariant::Cpu(prover) => {
                let (_, vk) = prover.setup(VEFAS_SP1_ELF);
                prover.verify(&sp1_proof, &vk).map_err(|e| {
            ...
            Sp1ProverVariant::Cuda(prover) => {
                let (_, vk) = prover.setup(VEFAS_SP1_ELF);
                prover.verify(&sp1_proof, &vk).map_err(|e| {
                    VefasError::zkvm_error("sp1", &format!("SP1 CUDA proof verification failed: {:?}", e))
                })?;
            }
        }
